function [net1, net2] = renjichuanbo2(changdu,kuandu,jiaoyoufanwei,frends, pengyoushu, net1,net2, fankang1, fankang2,zhixinfazhi,zhixindu,Adown,Bdown)
    ...
    for i = jiaoyoufanwei:1:changdu-jiaoyoufanwei-1
        ...
        % 在有小范围内进行遍历
            ...
            if net1(i,j) == 1 && net2(i,j) == 0
                if(zhouweizxd2> (zhixindu(i,j)) + 0.2 - Bdown * fankang2 * (rand()+0.5) )
                    net0(i,j) =-1;
                end
            end
            if net1(i,j) == 0 && net2(i,j) == 1
                if(zhouweizxd1> 1-zhixindu(i,j) + 0.2 - Adown * fankang1 * (rand()+0.5))
                    net0(i,j) = 1;
                end
            end
